...
class _OmniOutputTensorSnapshot(NamedTuple):
    ...
    deferred_payload: _AsyncCPUPayloadSnapshot | None = None
...
class GPUARModelRunner(OmniGPUModelRunner, OmniConnectorModelRunnerMixin):
    ...
    def _log_omni_async_output_defer_decision(self, should_defer_output: bool) -> None:
        model_config = getattr(self, "model_config", None)
        if model_config is None:
            model_config = getattr(getattr(self, "vllm_config", None), "model_config", None)
        model = getattr(self, "model", None)
    ...
    def _snapshot_omni_output_tensors_for_materialization(
        self,
        *,
        should_defer_output: bool,
        hidden_states: torch.Tensor,
        staged_hidden_states_cpu: torch.Tensor | None,
        multimodal_outputs: Any,
    ) -> _OmniOutputTensorSnapshot:
        if not should_defer_output or _use_unsafe_deferred_tensor_refs():
        ...
        if hidden_states_snapshot is None:
            # Models that omit hidden from the deferred snapshot only materialize
            # multimodal payloads (e.g. talker codes.audio).
        ...
        return _OmniOutputTensorSnapshot(
            ...
            deferred_payload=deferred_payload_snapshot,
        )
    ...
    def sample_tokens(
        ...
        self._log_omni_async_output_defer_decision(should_defer_output)
        output_tensor_snapshot = self._snapshot_omni_output_tensors_for_materialization(
            should_defer_output=should_defer_output,
            hidden_states=hidden_states,
            staged_hidden_states_cpu=staged_hidden_states_cpu,
            multimodal_outputs=multimodal_outputs,
        )
        ...
        def output_builder() -> OmniModelRunnerOutput:
            if output_tensor_snapshot.deferred_payload is not None:
                with record_function_or_nullcontext("omni_output_builder:wait_async_cpu_payload"):
                    output_tensor_snapshot.deferred_payload.wait()
            if _trace_deferred_output():
                logger.info(
